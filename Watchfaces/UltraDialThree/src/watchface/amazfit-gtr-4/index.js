...
try {
  (() => {
    ...
    const DEVICE_HEIGHT = 466
    const DEVICE_WIDTH = 466
      ...
      init_view() {
        ...
        for (let i = 0; i < 1; i++) {
          idle_second_TextCircle[i] = hmUI.createWidget(hmUI.widget.IMG, {
            ...
            center_x: 233,
            center_y: 233,
            pos_x: 233 - idle_second_TextCircle_img_width / 2,
            pos_y: 233 - 222,
            angle: 46,
            src: 'dig34w_dots.png',
            show_level: hmUI.show_level.ONLY_AOD,
        ...
        weatherGroup.createWidget(hmUI.widget.FILL_RECT, {
          x: 70 - 50,
          y: 280,
          w: 466 - 70 * 2,
          h: 100,
        ...
        //--------------------------------------------------------------------\\
        // for more info go to https://starrrscream.github.io/Website/index.html
        //--------------------------------------------------------------------\\

        function arrayBufferToCyrillic(buffer) {
          //-----------------------------------------------
          //big thx for arrayBufferToCyrillic to @SashaCX75
          //-----------------------------------------------
          let result = '';
        ...
        function getDataValue(str = "") {
          try {
            ...
            feelsLikeWidget.setProperty(hmUI.prop.TEXT, 'Feels like: ' + ` ${data.temperatureFeels}` + '°');
            conditionWidget.setProperty(hmUI.prop.TEXT, getWeatherDescription(data.weatherDescription, language));
            data.windSpeed != 0
              ? windWidget.setProperty(hmUI.prop.TEXT, `Wind: ${data.windSpeed}` + ` m/s, ` + getWindDirection(data.windTitle, language))
              : windWidget.setProperty(hmUI.prop.TEXT, "Wind: Calm")
            pressureWidget.setProperty(hmUI.prop.TEXT, `Pressure: ${data.pressure} mmHg`)


            wCity.setProperty(hmUI.prop.TEXT, data.city)
            wUpdate.setProperty(hmUI.prop.TEXT, 'Updated: ' + data.weatherTimeStr)

            normal_sun_high_text_font.setProperty(hmUI.prop.TEXT, data.sunriseTime)
            idle_sun_high_text_font.setProperty(hmUI.prop.TEXT, data.sunriseTime)
            normal_sun_low_text_font.setProperty(hmUI.prop.TEXT, data.sunsetTime)
            idle_sun_low_text_font.setProperty(hmUI.prop.TEXT, data.sunsetTime)
          ...
          function getWeatherDescription(state, language = 0) {
            const descriptions = {
              CLEAR: { 0: "Clear", 4: "Ясно" },
              PARTLY_CLOUDY: { 0: "Partly Cloudy", 4: "Переменная облачность" },
              CLOUDY: { 0: "Cloudy", 4: "Облачно" },
              OVERCAST: { 0: "Overcast", 4: "Пасмурно" },
              LIGHT_RAIN: { 0: "Light Rain", 4: "Небольшой дождь" },
              RAIN: { 0: "Rain", 4: "Дождь" },
              HEAVY_RAIN: { 0: "Heavy Rain", 4: "Сильный дождь" },
              SHOWERS: { 0: "Showers", 4: "Ливень" },
              SLEET: { 0: "Sleet", 4: "Дождь со снегом" },
              LIGHT_SNOW: { 0: "Light Snow", 4: "Небольшой снег" },
              SNOW: { 0: "Snow", 4: "Снег" },
              SNOWFALL: { 0: "Snowfall", 4: "Снегопад" },
              HAIL: { 0: "Hail", 4: "Град" },
              THUNDERSTORM: { 0: "Thunderstorm", 4: "Гроза" },
              THUNDERSTORM_WITH_RAIN: { 0: "Thunderstorm with Rain", 4: "Гроза с дождем" },
              THUNDERSTORM_WITH_HAIL: { 0: "Thunderstorm with Hail", 4: "Гроза с градом" },
              BLOWING_SNOW: { 0: "Blowing Snow", 4: "Метель" },
              DRIFTING_SNOW: { 0: "Drifting Snow", 4: "Слабая метель" },
              DRIZZLE: { 0: "Drizzle", 4: "Морось" },
              DUST: { 0: "Dust", 4: "Пыль" },
              DUST_SUSPENSION: { 0: "Dust Suspension", 4: "Пылевая взвесь" },
              DUSTSTORM: { 0: "Duststorm", 4: "Пыльная буря" },
              FOG: { 0: "Fog", 4: "Туман" },
              FREEZING_RAIN: { 0: "Freezing Rain", 4: "Ледяной дождь" },
              MIST: { 0: "Mist", 4: "Дымка" },
              SMOKE: { 0: "Smoke", 4: "Смог" },
              TORNADO: { 0: "Tornado", 4: "Торнадо" },
              VOLCANIC_ASH: { 0: "Volcanic Ash", 4: "Вулканический пепел" },
            };

            const defaultDescription = { 0: "Unknown State", 4: "Неизвестное состояние" };
            const description = descriptions[state] || defaultDescription;

            return description[language] || description[0]; // Если язык отсутствует, возвращаем значение по умолчанию (0)
          }
          function getWindDirection(abbreviation, language = 0) {
            switch (abbreviation) {
              case "N":
                return language === 4 ? "С" : "N";
              case "NE":
                return language === 4 ? "СВ" : "NE";
              case "E":
                return language === 4 ? "В" : "E";
              case "SE":
                return language === 4 ? "ЮВ" : "SE";
              case "S":
                return language === 4 ? "Ю" : "S";
              case "SW":
                return language === 4 ? "ЮЗ" : "SW";
              case "W":
                return language === 4 ? "З" : "W";
              case "NW":
                return language === 4 ? "СЗ" : "NW";
              default:
                return language === 4 ? "Неизвестное направление" : "Unknown direction";
            }
          }
